...
class Button:
    def __init__(self, button_title, position, width=BTN_WIDTH,
                 height=BTN_HEIGHT, background_color=BROWN, font_color=GREY):
...
class Display:
    def __init__(self, background_color=GREY, buttons=None, labels=None, sprites=None):
        if buttons is None:
            self.buttons = []
        else:
...
class UiManager:
    def __init__(self, screen, level):
        ...
        self.start_level_again_btn = Button('Начать сначала', SCREEN_CENTER, background_color=GREY, font_color=BLACK)
        self.lose_level_display = Display(BROWN, buttons=[self.start_level_again_btn])

        self.finish_btn = Button('Закончить', (WIDTH // 2, HEIGHT // 2 + 2 * BTN_HEIGHT))
        self.start_game_again_btn = Button('Начать сначала', SCREEN_CENTER)
        self.win_label = Label('Вы прошли игру!', (WIDTH // 2, HEIGHT // 2 - 2 * BTN_HEIGHT))
        self.win_game_display = Display(buttons=[self.start_game_again_btn, self.finish_btn], labels=[self.win_label])

        self.new_game_button = Button('Новая игра', SCREEN_CENTER, background_color=GREY, font_color=BLACK)
    ...
    def draw_button(self, button):
        ...
        title_params = (x_start + width / 2 - button.title_width / 2, y_start + height / 2 - button.title_height / 2)
