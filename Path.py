...
class Path:
    ...
    def change_direction(self, target_index, pos):
        direction = pygame.math.Vector2(self.targets[target_index][0] - pos[0], self.targets[target_index][1] - pos[1])
        length = math.hypot(*direction)
        direction = pygame.math.Vector2((direction[0] / length, direction[1] / length))
        return direction

    def draw(self, screen):
        for i in range(len(self.targets) - 1):
            pygame.draw.line(screen, BLACK, self.targets[i], self.targets[i + 1], 10)
